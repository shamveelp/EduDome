...
function InstituteLoginForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Optionally verify role here if backend returns it
      if (data.user && data.user.role !== 'AGENCY') {
         // This is a soft client-side warning/restriction. A real restriction would be handled by middleware or backend.
         console.warn("User logged in through Institute portal but does not have AGENCY role.");
      }
      ...
      <main className="flex-1 flex items-center justify-center py-12 px-6">
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100 w-full max-w-md">
          <div className="text-center mb-8">
            <h1 className="text-2xl font-bold text-[#1e293b] font-outfit">Institute Partner Login</h1>
            <p className="text-slate-500 mt-1 text-sm">Access your academy management dashboard</p>
          </div>
          ...
          <div className="text-center mt-6 text-slate-500 text-sm">
            Not an institute partner yet?{' '}
            <Link href="/institute/register" className="text-[#4C8CE4] font-medium hover:underline">
              Register here
            </Link>
          </div>
        </div>
